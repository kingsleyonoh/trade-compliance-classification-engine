...
$skipFilePatterns = @(
    ...
    '\.min\.(js|css)$',
    '\.bundle\.(js|css)$',
    '\.(png|jpg|jpeg|gif|webp|svg|ico|woff|woff2|ttf|otf|eot|pdf|zip|tar|gz|tgz|bz2|7z|exe|dll|so|dylib|class|jar|pyc|pyo|whl|mp3|mp4|mov|avi|stackdump)$',
    '(^|[\\/])\.pi[\\/]',
    '(^|[\\/])\.yolo[\\/](events|knowledge-packs|logs|runtime|subagent-events|worktrees)[\\/]',
    '[\\/]node_modules[\\/]',
    '[\\/]dist[\\/]',
    '[\\/]build[\\/]',
    '[\\/]\.next[\\/]',
